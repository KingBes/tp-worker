...
trait InteractsWithServer
{
    ...
    protected function guardAgainstDuplicateInstance(string $heartbeat): void
    {
        ...
        if ($pid > 0 && $this->isWinPhpProcessAlive($pid)) {
            throw new RuntimeException("tp-worker already running (master pid {$pid}), stop it before starting a new one.");
        }
    }
    ...
    protected function buildWindowsStartFile(string $root, string $type, string $masterToken): string
    {
        ...
        return <<<PHP
<?php
// auto-generated by tp-worker, do not edit
chdir(__DIR__ . '/../../');
...
if (!defined('TP_WORKER_MASTER_TOKEN')) {
    define('TP_WORKER_MASTER_TOKEN', {$tokenLiteral});
}

if (!in_array('-q', \$argv, true)) {
    // 本进程将成为 Workerman master（负责监控并重启各 -q 子进程），
    // 记录 PID 供孤儿子进程在 master 心跳丢失时回杀，避免重启风暴
    file_put_contents(__DIR__ . '/master_' . TP_WORKER_MASTER_TOKEN . '.pid', (string) getmypid());
}
